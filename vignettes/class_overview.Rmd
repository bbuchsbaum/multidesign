...
- **Cell mask** (`cells`): an optional logical matrix recording observed entries
...
A `hyperdesign` manages multiple `multidesign` objects (called "blocks") that share common design variables. This is the natural structure for multi-subject, multi-session, or multi-modal experiments. Optional `id` and `space` contracts also support blocks whose rows are corresponding entities in a shared column space.
...
For corresponding entities, construct with `id =` and `space = "common"`,
then use `align_by_id()` when an explicit join is needed. This is distinct from
`as_multidesign()`, which always row-stacks. The aligned result contains an
entity-by-variable-by-block data array, a same-shaped cell mask, and a separate
entity-by-block row-presence matrix. See the [Getting Started
vignette](Introduction.html#when-rows-correspond-across-blocks) for a runnable
